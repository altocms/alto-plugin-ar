...
class PluginAr_HookRepost extends Hook {
    ...
    public function AfterWallAdd($aData) {
        ...
        if ($oWall) {
            ...
            $oSettings = E::Module('User')->GetCurrentUserRepostSettingsByType('wall');
            if ($oSettings && $oProviders = E::Module('PluginAr\AuthProvider')->GetProvidersByTokenIdArray(array_keys($oSettings))) {
    ...
    public function AfterTopicAdd($aData) {
        ...
        if ($oTopic && $oTopic->getUserId() == E::UserId()) {
            ...
            // Получим социалки, для которых пользователь определи репост
            /** @var PluginAr_ModuleAuthProvider_EntityRepostSetting[] $oSettings */
            $oSettings = E::Module('User')->GetCurrentUserRepostSettingsByType('post');
            if ($oSettings && $oProviders = E::Module('PluginAr\AuthProvider')->GetProvidersByTokenIdArray(array_keys($oSettings))) {

                if (isset($oProviders['vk'])) {
                    E::Module('Session')->Set('new_current_user_topic_title', $oTopic->getTitle());
                    E::Module('Session')->Set('new_current_user_topic_id', $oTopic->getId());
                }
    ...
    public function AfterProfileSave($aData) {
        ...
        $sOldDataUserAbout = E::Module('Cache')->Get('current_user_profile_about', 'tmp');
        ...
        if ($oUser = (isset($aData['oUser']) ? $aData['oUser'] : FALSE)) {
            ...
            if ($oUser && E::IsUser() && $oUser->getId() == E::User()->getId()) {
                ...
                if ($sOldDataUserAbout != $oUser->getProfileAbout()) {
                    ...
                    // Получим социалки, для которых пользователь определи репост
                    /** @var PluginAr_ModuleAuthProvider_EntityRepostSetting[] $oSettings */
                    $oSettings = E::Module('User')->GetCurrentUserRepostSettingsByType('status');
                    if ($oSettings && $oProviders = E::Module('PluginAr\AuthProvider')->GetProvidersByTokenIdArray(array_keys($oSettings))) {
    ...
    public function BodyEndVKWall() {
        ...
        // топика и стены
        /** @var PluginAr_ModuleAuthProvider_EntityRepostSetting[] $oSettings */
        $oSettingsWall = E::Module('User')->GetCurrentUserRepostSettingsByType('wall');
        $oSettingsPost = E::Module('User')->GetCurrentUserRepostSettingsByType('post');
        ...
        if ($oSettings && $aProviders = E::Module('PluginAr\AuthProvider')->GetProvidersByTokenIdArray(array_keys($oSettings))) {
            ...
            if (isset($aProviders['vk'])) {
                ...
                $oLocalViewer->Assign('wall_good', E::Module('Lang')->Get('plugin.ar.wall_good'));
    ...
    public function BodyEndVKPost() {
        ...
        /** @var PluginAr_ModuleAuthProvider_EntityRepostSetting[] $oSettings */
        $oSettings = E::Module('User')->GetCurrentUserRepostSettingsByType('post');
        $sTopicTitle = E::Module('Session')->Get('new_current_user_topic_title', FALSE);
        $iTopicId = E::Module('Session')->Get('new_current_user_topic_id', FALSE);
        if ($sTopicTitle && $iTopicId && $oSettings && $aProviders = E::Module('PluginAr\AuthProvider')->GetProvidersByTokenIdArray(array_keys($oSettings))) {
            E::Module('Session')->Drop('new_current_user_topic_title');
            E::Module('Session')->Drop('new_current_user_topic_id');
            // Если среди провайдеров, у которых разрешена публикация
            // есть вконтакте, то запускаем скрипт
            if (isset($aProviders['vk'])) {
                /** @var ModuleViewer $oLocalViewer */
                ...
                $sText = E::Module('User')->GetCurrentUserText('topic');
                ...
                $oLocalViewer->Assign('post_good', E::Module('Lang')->Get('plugin.ar.post_good'));
